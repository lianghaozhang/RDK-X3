...
class Foxglove(Node):
    def __init__(self, name):
        ...
        # 订阅避障发布的ai数据
        self.sub_ai_msg = self.create_subscription(PerceptionTargets, "hobot_dnn_detection", self.callback_ai_msg, 1)
    ...
    def loop_callback(self):
        ...
        if self.flag == State.A1_LOW:
            self.global_ackermann_vel.drive.speed = 0.5
            self.global_ackermann_vel.drive.steering_angle = 3.0
            if time.time() - self.A1_LOW_startime >= 0.5:
